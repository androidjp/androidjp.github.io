...
--- | --- | --- | ---| ---| ---
ArrayList | 否 | 数组 | 10 | 1【当元素个数超过容量长度的100%时，进行扩容】 | 原容量 + 原容量>>1
Vector    | 是 | 数组 |  10 | 1 | 原容量 x 2
...
哦，其实此时，我们的table初始容量，并不是 `10000`，而是 `16384`。那么，阈值我们口算一下就能得到: `15384*0.75 = 12288`。所以，当我们插入1w条记录，HashMap的这段逻辑：
```
if (++size > threshold)
    resize();
```
